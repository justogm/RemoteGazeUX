...
class TestSubjectRepository:
    ...
    def test_create_subject(self, app):
        ...
        with app.app_context():
            ...
            subject = repo.create_subject(name="Juan", surname="Pérez", age=25)
            repo.commit()
    ...
    def test_create_subject_with_study(self, app):
        ...
        with app.app_context():
            ...
            subject = subject_repo.create_subject(
                name="María", surname="González", age=30, study_id=study.id
            )
            subject_repo.commit()
        ...
        with app.app_context():
            ...
            for i in range(3):
                ...
                measurement_repo.create_measurement(
                    date=datetime.now(), subject_id=subject1.id, gaze_point=point
                )

            # Create measurement for subject2
            point = point_repo.create_point(100.0, 200.0)
            measurement_repo.create_measurement(
                date=datetime.now(), subject_id=subject2.id, gaze_point=point
            )
            measurement_repo.commit()
...
class TestTaskLogRepository:
    ...
    def test_get_tasklogs_by_subject(self, app):
        ...
        with app.app_context():
            ...
            tasklog_repo.create_tasklog(
                start_time=datetime.now(), subject_id=subject2.id, response="Response"
            )
            tasklog_repo.commit()
    ...
    def test_count_tasklogs_by_subject(self, app):
        ...
        with app.app_context():
            ...
            for i in range(5):
                tasklog_repo.create_tasklog(
                    start_time=datetime.now(), subject_id=subject.id
                )
            tasklog_repo.commit()
