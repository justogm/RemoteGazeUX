...
class TestSubjectModel:
    ...
    def test_subject_with_study(self, app):
        ...
        with app.app_context():
            ...
            subject = Subject(name="María", surname="García", age=30, study_id=study.id)
            db.session.add(subject)
            db.session.commit()
...
class TestMeasurementModel:
    ...
    def test_measurement_str(self, app):
        ...
        with app.app_context():
            ...
            measurement = Measurement(date=datetime.now(), subject_id=subject.id)
            db.session.add(measurement)
            db.session.commit()
...
class TestTaskLogModel:
    ...
    def test_tasklog_str(self, app):
        ...
        with app.app_context():
            ...
            tasklog = TaskLog(start_time=datetime.now(), subject_id=subject.id)
            db.session.add(tasklog)
            db.session.commit()
